...
export async function singlePaymentDOT(account, destination, value) { 
    ...
    try {
          ...
          const userConfirmed = confirm(`Please, confirm payment of ${formatConversionOut(value, 10)} DOT to beneficiary ${destination}.
          Estimated fee: ${formatConversionOut(txFee, 10)} DOT`);
          ...
          //Verify DOT balance for transaction
          const totalRequired = value.add(MIN_BAL_FREE['DOT']).add(txFee.muln(2));

          if (balances['DOT'].lt(totalRequired)){
